...
from skyflow.network.cluster_linkv2 import CL_DIRECTORY, launch_clusterlink, create_link, export_service, import_service
...
def load_services():
    ...
    os.system(f"kubectl run iperf3-server --image {test_service} --port 5000 -l app=iperf3-server -- -s -p 5000")
...
def try_connection():
    ...
    try:
        ...
        direct_output = subprocess.check_output(f"kubectl exec -i iperf3-client -- iperf3 -c iperf3-server-{cl2Name} -p {destPort}", shell=True)
...
def test_create_cluster():
    ...
    load_services()
    assert export_service(cluster2_service, cluster2_manager, [destPort]) is True
    assert import_service(cluster2_service, cluster1_manager, cluster2_manager.cluster_name, [destPort]) is True
