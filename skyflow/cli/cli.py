...
def create_job(name, namespace, labels, image, envs, cpus, gpus, accelerators,
               memory, run, replicas, restart_policy):
    """Adds a new job."""
    labels = dict(labels)
    envs = dict(envs)
...
def create_service(name: str, namespace: str, type: str,
                   selector: List[Tuple[str, str]],
                   ports: List[Tuple[int, int]], cluster: str):
