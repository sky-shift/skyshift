...
class KubernetesManager(Manager):
    ...
    def _convert_to_deployment_yaml(self, job: Job, job_name: Optional[str]):
        """
        ...
        if gpus == 0:
            ...
            if len(job_resource_dict) != 0:
                assert len(job_resource_dict
                           ) == 1, "Job can only specify one accelerator type."
    ...
    def _convert_to_pod_yaml(self, job: Job, job_name: Optional[str] = None):
        """
    ...
    def create_or_update_service(self,
                                 service: Service,
                                 override_name: Optional[str] = None) -> None:
        dir_path = os.path.dirname(os.path.realpath(__file__))
        jinja_env = Environment(loader=FileSystemLoader(
            os.path.abspath(dir_path)),
                                autoescape=select_autoescape())
        ...
        for cluster_name, endpoint_obj in endpoints.spec.endpoints.items():
            ...
            if endpoint_obj.exposed_to_cluster:
                ...
                for _ in range(endpoint_obj.num_endpoints):
                    ...
                    ports: List[int] = [p.port for p in subset.ports]
