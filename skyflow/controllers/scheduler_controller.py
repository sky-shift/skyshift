...
def rank_clusters(job: Job, clusters: dict):
    ...
    Whatever scores highest on the **dot product** of the
    cluster's resources and the job's resources
    (such as in the Tetris paper, Robert Grandl).
    """
    sum_clusters = []
...
def filter_clusters(job: Job, clusters: dict):
    """
    Filters for clusters that satisfy the job's filter policies.
    """
    # Filter for clusters.
...
class SchedulerController(Controller):
    ...
    def controller_loop(self):
        ...
        for job_idx, job in enumerate(self.workload_queue):
            ...
            filtered_clusters = filter_clusters(job, clusters)
