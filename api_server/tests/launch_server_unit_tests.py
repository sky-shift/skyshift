...
class TestLaunchAPIServer(unittest.TestCase):
    ...
    def test_generate_manager_config(self, mock_file, mock_makedirs):
        ...
        expected_file_path = os.path.expanduser(launch_server.API_SERVER_CONFIG_PATH)
    ...
    def test_check_and_install_etcd(self, mock_popen, mock_run):
        ...
        relative_dir = os.path.dirname(os.path.realpath(launch_server.__file__))
        mock_popen.assert_called_with(
            f"{relative_dir}/install_etcd.sh",
            shell=True,
    ...
    def test_main(self, mock_check_etcd, mock_generate_config, mock_uvicorn):
        ...
        test_host = "127.0.0.1"
        test_port = 8080
        launch_server.main(test_host, test_port)

        mock_check_etcd.assert_called_once()
        mock_generate_config.assert_called_with(test_host, test_port)
        mock_uvicorn.assert_called_with("api_server:app", host=test_host, port=test_port, workers=multiprocessing.cpu_count())

    def test_parse_args(self):
        """
        Test the parse_args function for correct argument parsing.
        """
        test_args = ['script_name', '--host', '127.0.0.1', '--port', '8080']
