...
## Building Slurmrestd on host system
Pull the latest version of slurm on respective distribution's package manager eg.
```
sudo apt-get slurm-wlm
```
Get the slurm version
```
sudo slurmctld -V
```
Use this version number to pass to docker compose build script.
Slurm version on host system must be the same on the container.
...
```
## Starting Slurmrestd on host system
Slurmrestd depends on slurmdbd to be up inside the container wait until it has initialized.
Run 
```console 
slurmrestd -s {openapi_version} unix:{path_to_unix_socket} -a rest_auth/local -vvv
```
Ping slurmrestd to see if connection reached slurmctld
```console 
curl --unix-socket "${SLURMRESTD}" 'http://localhost:8080/slurm/v0.0.38/node/{nodename}'
```
Get the status of the head node.
```console 
curl --unix-socket "{path_to_unix_socket}" 'http://localhost:8080/slurm/{openapi_version}/node/c1'
```
