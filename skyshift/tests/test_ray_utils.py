"""
Unit tests for the Ray utility functions in the SkyShift project.
"""
...
from skyshift.cluster_manager.ray.ray_utils import (
    copy_file_to_remote, copy_required_files, create_archive, extract_job_name,
    fetch_all_job_statuses, get_remote_home_directory,
    map_ray_status_to_task_status, process_cluster_status)
...
def test_copy_file_to_remote():
    ...
    with patch("skyshift.cluster_manager.ray.ray_utils.ssh_send_command"
               ) as mock_ssh_send_command:
        with patch("paramiko.SSHClient.open_sftp",
                   return_value=MagicMock()) as _:

            copy_file_to_remote(ssh_client, local_path, remote_path)

            mock_ssh_send_command.assert_called_once_with(
                ssh_client, "mkdir -p /remote/path/to")
...
def test_process_cluster_status_missing_resources():
    ...
    status_dict = json.loads(json.dumps(BASE_STATUS_DICT))
    status_dict["data"]["clusterStatus"]["loadMetricsReport"]["usageByNode"][
        "node1"] = {
            "CPU": [1, 4]
        }
    result = process_cluster_status(status_dict, usage=True)
    expected = {"node1": {ResourceEnum.CPU.value: 3}}
    assert result == expected
...
def test_process_cluster_status_mixed_gpu_types():
    ...
    status_dict = json.loads(json.dumps(BASE_STATUS_DICT))
    status_dict["data"]["clusterStatus"]["loadMetricsReport"]["usageByNode"][
        "node1"]["accelerator_type:NVIDIA_TESLA_K80"] = [0, 2]
    result = process_cluster_status(status_dict, usage=False)
    expected = {
        "node1": {
            ResourceEnum.CPU.value: 4,
...
def test_fetch_all_job_statuses_same_name():
    ...
    job_details = [
        MagicMock(submission_id="job-1-1234",
                  job_id="job-1-1234",
                  status="RUNNING"),
        MagicMock(submission_id="job-2-5678",
                  job_id="job-2-5678",
                  status="SUCCEEDED")
    ]
...
def test_fetch_all_job_statuses_different_names():
    ...
    job_details = [
        MagicMock(submission_id="job1-1-1234",
                  job_id="job1-1-1234",
                  status="RUNNING"),
...
def test_fetch_all_job_statuses_invalid_submission_ids():
    ...
    job_details = [
        MagicMock(submission_id="invalid-1234",
                  job_id="job-1-1234",
                  status="RUNNING"),
        MagicMock(submission_id="job2-2-5678",
                  job_id="job2-5678",
...
def test_copy_required_files():
    ...
    with patch("skyshift.cluster_manager.ray.ray_utils.create_archive"
               ) as mock_create_archive:
        with patch("skyshift.cluster_manager.ray.ray_utils.copy_file_to_remote"
                   ) as mock_copy_file_to_remote:
            with patch(
                    "skyshift.cluster_manager.ray.ray_utils.extract_archive_on_remote"
            ) as mock_extract_archive_on_remote:
                with patch("skyshift.cluster_manager.ray.ray_utils.os.remove"
                           ) as mock_os_remove:
